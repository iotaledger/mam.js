...
test("createChannel() fails with empty seed", () => {
    expect(() => createChannel(undefined as never, 0, undefined as never)).toThrow("The seed");
});

test("createChannel() fails with invalid seed", () => {
    expect(() => createChannel("aaa", 0, undefined as never)).toThrow("The seed");
});

test("createChannel() fails with security level too low", () => {
    expect(() => createChannel("A".repeat(81), 0, undefined as never)).toThrow("Security must");
});

test("createChannel() fails with security level too high", () => {
    expect(() => createChannel("A".repeat(81), 4, undefined as never)).toThrow("Security must");
});

test("createChannel() fails invalid mode", () => {
    expect(() => createChannel("A".repeat(81), 1, "foo" as never)).toThrow("The mode");
});
