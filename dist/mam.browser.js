(function (global, factory) {
	...
	Object.defineProperty(textHelper, "__esModule", { value: true });
	textHelper.TextHelper = void 0;
	...
	Object.defineProperty(trytesHelper, "__esModule", { value: true });
	trytesHelper.TrytesHelper = void 0;
	// Copyright 2021 IOTA Stiftung
	// SPDX-License-Identifier: Apache-2.0
	const iota_js_1$6 = require$$0__default['default'];
	const textHelper_1 = textHelper;
	/**
	...
	class TrytesHelper {
	    ...
	    static packTrytes(trytes) {
	        ...
	        return iota_js_1$6.Converter.binaryToBytes(allBits);
	    }
	    /**
	    ...
	    static unpackTrytes(packed) {
	        const allBits = iota_js_1$6.Converter.bytesToBinary(packed);
	        const trytes = [];
	...
	trytesHelper.TrytesHelper = TrytesHelper;
	/**
	...
	Object.defineProperty(merkleHashGenerator, "__esModule", { value: true });
	merkleHashGenerator.generateAddress = void 0;
	// Copyright 2021 IOTA Stiftung
	// SPDX-License-Identifier: Apache-2.0
	const iss_p27_1$2 = issP27;
	/**
	...
	function generateAddress(seedTrits, index, security) {
	    const ss = iss_p27_1$2.subseed(seedTrits, index);
	    const dg = iss_p27_1$2.digestFromSubseed(ss, security);
	    return {
	        address: iss_p27_1$2.address(dg),
	        privateKey: iss_p27_1$2.privateKeyFromSubseed(ss, security)
	    };
	}
	merkleHashGenerator.generateAddress = generateAddress;

	var merkleNode = {};

	Object.defineProperty(merkleNode, "__esModule", { value: true });
	merkleNode.MerkleNode = void 0;
	// Copyright 2021 IOTA Stiftung
	// SPDX-License-Identifier: Apache-2.0
	/**
	...
	merkleNode.MerkleNode = MerkleNode;

	Object.defineProperty(merkleTree, "__esModule", { value: true });
	merkleTree.MerkleTree = void 0;
	// Copyright 2021 IOTA Stiftung
	// SPDX-License-Identifier: Apache-2.0
	const iota_js_1$5 = require$$0__default['default'];
	const trytesHelper_1$5 = trytesHelper;
	const merkleHashGenerator_1 = merkleHashGenerator;
	const merkleNode_1 = merkleNode;
	/**
	...
	class MerkleTree {
	    ...
	    constructor(seed, index, count, security) {
	        const seedTrits = trytesHelper_1$5.TrytesHelper.toTrits(seed);
	        const leaves = [];
	    ...
	    static root(rate, siblings, index) {
	        const sponge = new iota_js_1$5.Curl(27);
	        let i = 1;
	        const numChunks = Math.ceil(siblings.length / iota_js_1$5.Curl.HASH_LENGTH);
	        for (let c = 0; c < numChunks; c++) {
	            const chunk = siblings.slice(c * iota_js_1$5.Curl.HASH_LENGTH, (c + 1) * iota_js_1$5.Curl.HASH_LENGTH);
	            sponge.reset();
	    ...
	    buildTree(leaves) {
	        ...
	        for (let i = 0; i < leaves.length; i += 2) {
	            ...
	            if (right) {
	                ...
	                addressTrits = new Int8Array(iota_js_1$5.Curl.HASH_LENGTH);
	                sponge.squeeze(addressTrits, 0, addressTrits.length);
	            }
	            else {
	...
	Object.defineProperty(pascal, "__esModule", { value: true });
	pascal.roundThird = pascal.pascalDecode = pascal.pascalEncode = void 0;
	// Copyright 2021 IOTA Stiftung
	// SPDX-License-Identifier: Apache-2.0
	const trytesHelper_1$4 = trytesHelper;
	...
	function pascalEncode(value) {
	    ...
	    for (let i = 0; i < length - TRITS_PER_TRYTE; i += TRITS_PER_TRYTE) {
	        ...
	        const tritsAsInt = trytesHelper_1$4.TrytesHelper.tritsValue(tritValue);
	        if (tritsAsInt >= 0) {
	            encoding |= 1 << index;
	    ...
	    if (trytesHelper_1$4.TrytesHelper.tritsValue(v) < 0) {
	        encoding |= 1 << index;
	...
	pascal.pascalEncode = pascalEncode;
	/**
	...
	function pascalDecode(value) {
	    ...
	    const encoder = trytesHelper_1$4.TrytesHelper.tritsValue(value.slice(encoderStart, inputEnd));
	    let result = 0;
	    for (let i = 0; i < encoderStart / TRITS_PER_TRYTE; i++) {
	        const tritsIntValue = ((encoder >> i) & 1) !== 0
	            ? -trytesHelper_1$4.TrytesHelper.tritsValue(value.slice(i * TRITS_PER_TRYTE, (i + 1) * TRITS_PER_TRYTE))
	            : trytesHelper_1$4.TrytesHelper.tritsValue(value.slice(i * TRITS_PER_TRYTE, (i + 1) * TRITS_PER_TRYTE));
	        result += (Math.pow(27, i) * tritsIntValue);
	    }
	    return { value: result, end: inputEnd };
	}
	pascal.pascalDecode = pascalDecode;
	/**
	 ...
	 */
	function end(input) {
	    if (trytesHelper_1$4.TrytesHelper.tritsValue(input.slice(0, TRITS_PER_TRYTE)) > 0) {
	...
	Object.defineProperty(hammingDiver, "__esModule", { value: true });
	hammingDiver.HammingDiver = void 0;
	// Copyright 2021 IOTA Stiftung
	// SPDX-License-Identifier: Apache-2.0
	const iota_js_1$4 = require$$0__default['default'];
	const big_integer_1 = __importDefault(require$$1__default['default']);
	const pascal_1$2 = pascal;
	/**
	...
	class HammingDiver {
	    ...
	    search(trits, securityLevel, length, offset) {
	        ...
	        let size = Math.min(length, iota_js_1$4.Curl.HASH_LENGTH) - offset;
	        let index = 0;
	        while (index === 0) {
	            const incrementResult = this.increment(state, offset + (size * 2 / 3), offset + size);
	            size = Math.min(pascal_1$2.roundThird(offset + (size * 2 / 3) + incrementResult), iota_js_1$4.Curl.HASH_LENGTH) - offset;
	            const curlCopy = {
	                low: state.low.slice(),
	    ...
	    prepareTrits(trits, offset) {
	        const initialState = this.tritsToBigInt(trits, iota_js_1$4.Curl.STATE_LENGTH);
	    ...
	    transform(searchStates) {
	        ...
	        for (let round = 0; round < HammingDiver.ROUNDS; round++) {
	            const curlScratchpad = {
	                low: searchStates.low.slice(0, iota_js_1$4.Curl.STATE_LENGTH),
	                high: searchStates.high.slice(0, iota_js_1$4.Curl.STATE_LENGTH)
	            };
	            for (let stateIndex = 0; stateIndex < iota_js_1$4.Curl.STATE_LENGTH; stateIndex++) {
	...
	hammingDiver.HammingDiver = HammingDiver;
	/**
	 * Max 64 bit value.
	 */
	...
	arrayHelper.concatenate = void 0;
	// Copyright 2021 IOTA Stiftung
	// SPDX-License-Identifier: Apache-2.0
	/**
	...
	arrayHelper.concatenate = concatenate;

	var guards = {};

	Object.defineProperty(guards, "__esModule", { value: true });
	guards.validateModeKey = void 0;
	const trytesHelper_1$3 = trytesHelper;
	/**
	...
	function validateModeKey(mode, sideKey) {
	    ...
	    if (mode === "restricted") {
	        ...
	        if (!trytesHelper_1$3.TrytesHelper.isTrytes(sideKey)) {
	...
	guards.validateModeKey = validateModeKey;

	var mask$1 = {};

	Object.defineProperty(mask$1, "__esModule", { value: true });
	mask$1.unmask = mask$1.mask = mask$1.maskHash = void 0;
	// Copyright 2021 IOTA Stiftung
	// SPDX-License-Identifier: Apache-2.0
	const iota_js_1$3 = require$$0__default['default'];
	/**
	...
	function maskHash(keyTrits) {
	    const sponge = new iota_js_1$3.Curl(81);
	    sponge.absorb(keyTrits, 0, keyTrits.length);
	    const finalKeyTrits = new Int8Array(iota_js_1$3.Curl.HASH_LENGTH);
	    sponge.squeeze(finalKeyTrits, 0, finalKeyTrits.length);
	    return finalKeyTrits;
	}
	mask$1.maskHash = maskHash;
	/**
	...
	function mask(payload, sponge) {
	    ...
	    const numChunks = Math.ceil(payload.length / iota_js_1$3.Curl.HASH_LENGTH);
	    for (let c = 0; c < numChunks; c++) {
	        const chunk = payload.slice(c * iota_js_1$3.Curl.HASH_LENGTH, (c + 1) * iota_js_1$3.Curl.HASH_LENGTH);
	        sponge.absorb(chunk, 0, chunk.length);
	        const state = sponge.rate();
	        for (let i = 0; i < chunk.length; i++) {
	            payload[(c * iota_js_1$3.Curl.HASH_LENGTH) + i] = tritSum(chunk[i], keyChunk[i]);
	            keyChunk[i] = state[i];
	        }
	    }
	    return payload;
	...
	function unmask(payload, sponge) {
	    ...
	    const limit = Math.ceil(unmasked.length / iota_js_1$3.Curl.HASH_LENGTH) * iota_js_1$3.Curl.HASH_LENGTH;
	    let state;
	    for (let c = 0; c < limit; c++) {
	        const indexInChunk = c % iota_js_1$3.Curl.HASH_LENGTH;
	        if (indexInChunk === 0) {
	            state = sponge.rate();
	        }
	        if (state) {
	        ...
	        if (indexInChunk === iota_js_1$3.Curl.HASH_LENGTH - 1) {
	            sponge.absorb(unmasked, Math.floor(c / iota_js_1$3.Curl.HASH_LENGTH) * iota_js_1$3.Curl.HASH_LENGTH, iota_js_1$3.Curl.HASH_LENGTH);
	        }
	    }
	    return unmasked;
	}
	mask$1.unmask = unmask;
	/**
	...
	channel.createMessage = channel.channelRoot = channel.createChannel = void 0;
	// Copyright 2021 IOTA Stiftung
	// SPDX-License-Identifier: Apache-2.0
	const iota_js_1$2 = require$$0__default['default'];
	const merkleTree_1$1 = merkleTree;
	const hammingDiver_1 = hammingDiver;
	const iss_p27_1$1 = issP27;
	const arrayHelper_1 = arrayHelper;
	const guards_1$1 = guards;
	const mask_1$2 = mask$1;
	const pascal_1$1 = pascal;
	const trytesHelper_1$2 = trytesHelper;
	/**
	...
	function channelRoot(channelState) {
	    ...
	    const tree = new merkleTree_1$1.MerkleTree(channelState.seed, channelState.start, channelState.count, channelState.security);
	    return trytesHelper_1$2.TrytesHelper.fromTrits(tree.root.addressTrits);
	}
	channel.channelRoot = channelRoot;
	/**
	...
	function createMessage(channelState, message) {
	    var _a;
	    if (!trytesHelper_1$2.TrytesHelper.isTrytes(message)) {
	        throw new Error("The message must be in trytes");
	    }
	    const tree = new merkleTree_1$1.MerkleTree(channelState.seed, channelState.start, channelState.count, channelState.security);
	    const nextRootTree = new merkleTree_1$1.MerkleTree(channelState.seed, channelState.start + channelState.count, channelState.nextCount, channelState.security);
	    const nextRootTrits = nextRootTree.root.addressTrits;
	    const messageTrits = trytesHelper_1$2.TrytesHelper.toTrits(message);
	    const indexTrits = pascal_1$1.pascalEncode(channelState.index);
	    const messageLengthTrits = pascal_1$1.pascalEncode(messageTrits.length);
	    const subtree = tree.getSubtree(channelState.index);
	    const sponge = new iota_js_1$2.Curl(27);
	    const sideKeyTrits = trytesHelper_1$2.TrytesHelper.toTrits((_a = channelState.sideKey) !== null && _a !== void 0 ? _a : "9".repeat(81));
	    ...
	    const maskedNextRoot = mask_1$2.mask(arrayHelper_1.concatenate([nextRootTrits, messageTrits]), sponge);
	    ...
	    const nonceTrits = hammingDiver.search(sponge.rate(iota_js_1$2.Curl.STATE_LENGTH), channelState.security, iota_js_1$2.Curl.HASH_LENGTH / 3, 0);
	    mask_1$2.mask(nonceTrits, sponge);
	    payload = arrayHelper_1.concatenate([payload, nonceTrits]);
	    // Create the signature and add the sibling information
	    const sig = iss_p27_1$1.signature(sponge.rate(), subtree.key);
	    const subtreeTrits = arrayHelper_1.concatenate(subtree.leaves.map(l => l.addressTrits));
	    const siblingsCount = subtreeTrits.length / iota_js_1$2.Curl.HASH_LENGTH;
	    const encryptedSignature = mask_1$2.mask(arrayHelper_1.concatenate([sig, pascal_1$1.pascalEncode(siblingsCount), subtreeTrits]), sponge);
	    ...
	    const messageAddress = channelState.mode === "public"
	        ? tree.root.addressTrits : mask_1$2.maskHash(tree.root.addressTrits);
	    const maskedAuthenticatedMessage = {
	        payload: trytesHelper_1$2.TrytesHelper.fromTrits(payload),
	        root: trytesHelper_1$2.TrytesHelper.fromTrits(tree.root.addressTrits),
	        address: trytesHelper_1$2.TrytesHelper.fromTrits(messageAddress)
	    };
	    ...
	    channelState.nextRoot = trytesHelper_1$2.TrytesHelper.fromTrits(nextRootTrits);
	    return maskedAuthenticatedMessage;
	}
	channel.createMessage = createMessage;
	...
	parser.parseMessage = void 0;
	// Copyright 2021 IOTA Stiftung
	// SPDX-License-Identifier: Apache-2.0
	const iota_js_1$1 = require$$0__default['default'];
	const merkleTree_1 = merkleTree;
	const iss_p27_1 = issP27;
	const mask_1$1 = mask$1;
	const pascal_1 = pascal;
	const trytesHelper_1$1 = trytesHelper;
	/**
	...
	function parseMessage(payload, root, channelKey) {
	    const payloadTrits = trytesHelper_1$1.TrytesHelper.toTrits(payload);
	    const rootTrits = trytesHelper_1$1.TrytesHelper.toTrits(root);
	    const channelKeyTrits = trytesHelper_1$1.TrytesHelper.toTrits(channelKey !== null && channelKey !== void 0 ? channelKey : "9".repeat(81));
	    ...
	    const messageStart = nextRootStart + iota_js_1$1.Curl.HASH_LENGTH;
	    const messageEnd = messageStart + messageLength;
	    // Hash the key, root and payload
	    const sponge = new iota_js_1$1.Curl(27);
	    ...
	    const nextRoot = mask_1$1.unmask(payloadTrits.slice(nextRootStart, nextRootStart + iota_js_1$1.Curl.HASH_LENGTH), sponge);
	    const message = mask_1$1.unmask(payloadTrits.slice(messageStart, messageStart + messageLength), sponge);
	    const nonce = mask_1$1.unmask(payloadTrits.slice(messageEnd, messageEnd + (iota_js_1$1.Curl.HASH_LENGTH / 3)), sponge);
	    ...
	    const decryptedMetadata = mask_1$1.unmask(payloadTrits.slice(messageEnd + nonce.length), sponge);
	    sponge.reset();
	    ...
	    if (siblingsCount !== 0) {
	        ...
	        const siblings = decryptedMetadata.slice(siblingsStart, siblingsStart + (siblingsCount * iota_js_1$1.Curl.HASH_LENGTH));
	    ...
	    if (trytesHelper_1$1.TrytesHelper.fromTrits(recalculatedRoot) !== root) {
	        throw new Error("Signature did not match expected root");
	    }
	    return {
	        nextRoot: trytesHelper_1$1.TrytesHelper.fromTrits(nextRoot),
	        message: trytesHelper_1$1.TrytesHelper.fromTrits(message)
	    };
	}
	...
	client.decodeMessages = client.mamFetchAll = client.decodeAddress = client.mamFetch = client.mamAttach = void 0;
	// Copyright 2021 IOTA Stiftung
	// SPDX-License-Identifier: Apache-2.0
	const iota_js_1 = require$$0__default['default'];
	const guards_1 = guards;
	const mask_1 = mask$1;
	const trytesHelper_1 = trytesHelper;
	...
	client.decodeAddress = decodeAddress;
	/**
	...
	client.mamFetchAll = mamFetchAll;
	/**
	...
	client.decodeMessages = decodeMessages;

	var IMamChannelState = {};

	Object.defineProperty(IMamChannelState, "__esModule", { value: true });

	var IMamFetchedMessage = {};

	Object.defineProperty(IMamFetchedMessage, "__esModule", { value: true });

	var IMamMessage = {};

	Object.defineProperty(IMamMessage, "__esModule", { value: true });

	var mamMode = {};

	Object.defineProperty(mamMode, "__esModule", { value: true });
